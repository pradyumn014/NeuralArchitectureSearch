...
class hill_climbing(object):
    def __init__(self, iterations = 10, branching = 10, net = None):
        ...
        if net:
            ...
            m1.describe_adj_list([flatten], [n1])
            flatten.describe_adj_list([m1], [f1])
            f1.describe_adj_list([flatten], [])
            self.net = Network({0:[[], [1]], 1:[[0], [2]], 2: [[1], [3]], 3:[[2], []]}, {0: n1, 1:m1, 2:flatten, 3:f1}, 'test_net')
    ...
    def start(self):
        ...
        for iter in range(self.iterations):
            ...
            par_loss, _ = Train(current_net, 1, 128, 0.01, 0.001)
            ...
            for no in range(self.branching):
                ...
                loss, _ = Train(child, 1, 128, 0.01, 0.001)
